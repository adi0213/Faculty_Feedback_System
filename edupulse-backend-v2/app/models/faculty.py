...
class FacultyProfile(Base):
    ...
    user_id: str = Column(String(36), nullable=False, unique=True)  # FK to registry.users (not enforced cross-schema)
    faculty_code: str = Column(String(20), nullable=False, unique=True)  # Opaque: FAC-NNNN
    name: str = Column(String(200), nullable=False)
    dept: str = Column(String(100), nullable=False)
    subject: str | None = Column(String(200), nullable=True)
    college: str = Column(String(200), nullable=False)
    university: str = Column(String(200), nullable=False, default="KTU")

    # Aggregated statistics (recomputed by scoring engine after each batch)
    response_count: int = Column(Integer, default=0, nullable=False)

    # Per-dimension running means (JSON: {"clarity": 3.8, ...})
    dimension_scores_json: str = Column(Text, default="{}", nullable=False)

    # Historical composite scores per term (JSON: [{"term": "2024-S1", "score": 3.6}, ...])
    score_trend_json: str = Column(Text, default="[]", nullable=False)

    # Current composite (Empirical Bayes shrunk, Winsorized)
    composite_score: float | None = Column(Float, nullable=True)
    score_band: str | None = Column(String(20), nullable=True)  # strong | developing | needs_support

    # Flags
    has_anomaly_flag: bool = Column(Boolean, default=False)
    anomaly_details_json: str | None = Column(Text, nullable=True)
